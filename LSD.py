...
if type_of_data == "Mitochondrial":
    ...
    # New question to ask whether it's V4 or V1V2 for bacterial data
    bacterial_region = questionary.select(
        "Please specify the region of 16S you're working with:",
        choices=["V4", "V1V2"]
    ).ask()

    if bacterial_region == "V4":
        n = 250
        m = 400
    elif bacterial_region == "V1V2":
        n = 250
        m = 360
...
'''
# Match files containing '_R1' for forward reads
r1_files = [f for f in files if '_R1.fq' in f]

# Match files containing '_R2' for reverse reads
r2_files = [f for f in files if '_R2.fq' in f]

# Process each R1 file to find the corresponding R2 file
for r1_file in r1_files:
    # Extract the sample name by removing '_R1.fq' from the forward read filename
    sample_name = r1_file.replace('_R1.fq', '')

    # Construct the reverse read filename by replacing '_R1.fq' with '_R2.fq'
    r2_file = f"{sample_name}_R2.fq"
    ...
    if r2_file in r2_files:
        ...
        print(f"Missing reverse file (R2) for: {r1_file}")
'''
os.chdir(output_dir)
'''
...
if type_of_data == "Mitochondria":
    os.system("""for file in *.fasta; do
        ...
        sed -E "s/.*CC[ACT]GA[TC]AT[AG]GC[ACT]TT[TC]CC[ACT]CG//; s/TG[AG]TT[TC]TT[TC]GG[ACGT]CA[TC]CC[ACT]G.*//" > "$SampleName"_raw.fasta
    done""")
elif type_of_data == "Bacterial":
    if bacterial_region == "V4":
        os.system("""for file in *.fasta; do
            ...
            sed -E 's/.*GTG[TC]CAGC[CA]GCCGCGGTAA//; s/ATTAGA[AT]ACCC[ACGT][ACGT]GTAGTCC.*//' > "$SampleName"_raw.fasta
        done""")
    elif bacterial_region == "V1V2":
        os.system("""for file in *.fasta; do
            SampleName=`basename $file .fasta`
        	egrep -B 1 "AG[AC]GTT[TC]GAT[TC][AC]TGGCTCAG.{250,350}ACTCCTACGGGAGGCAGCA" $SampleName.fasta | egrep -v "^\-\-$" |
        	sed -E 's/.*AG[AC]GTT[TC]GAT[TC][AC]TGGCTCAG//; s/ACTCCTACGGGAGGCAGCA.*//' > "$SampleName"_raw.fasta        
        done""")
    
print("OK!")   
###Deleting untrimmed sequences:

os.system('find . -type f -name "*.fasta" ! -name "*_raw.fasta" -delete')
...
os.system("""for file in *sorted.fasta; do
    ...
    usearch -unoise3 $SampleName.sorted.fasta -zotus $SampleName.zotus.fasta -tabbedout $SampleName.denoising.summary.txt -minsize 8
done""")
...
for zOTU in zOTU_list:
    ...
    # Debugging: print the current zOTU and its corresponding dictionary content
    print(f"Debug: {zOTU} => {zOTU_dict[zOTU]}")  # This helps check what data is being processed
    
    # Ensure the list has the correct number of elements (i.e., indices 1, 2, and 3 exist)
    if len(zOTU_dict[zOTU]) < 4:
        print(f"Warning: zOTU {zOTU} is missing data. Skipping this zOTU. Data: {zOTU_dict[zOTU]}")
        continue  # Skip this iteration since the zOTU does not have enough data

    # Handle missing data gracefully
    taxonomy = zOTU_dict[zOTU][1] if len(zOTU_dict[zOTU]) > 1 else "unassigned"
    sequence = zOTU_dict[zOTU][2] if len(zOTU_dict[zOTU]) > 2 else "N/A"
    otu_assignment = zOTU_dict[zOTU][3] if len(zOTU_dict[zOTU]) > 3 else "N/A"
    
    # Output the row with valid or default values
    print(zOTU, otu_assignment, taxonomy, sequence, Total, sep = "\t", end = "\t", file = OUTPUT_TABLE)
