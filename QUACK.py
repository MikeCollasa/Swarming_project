...
header = Statistics[0]  # Get the header from the first row

# Create a list of column names to delete
columns_to_delete = [item for item in header if item in Blank_PCR or item in Blank_Extr]

# Create a list of indices to delete columns from the rows in Statistics
indices_to_delete = [header.index(column_name) for column_name in columns_to_delete]

# Delete the columns from the header
header = [item for item in header if item not in columns_to_delete]

# Delete the columns from all rows in the Statistics list (iterating backwards)
